...
export default function Footer() {
  ...
  const isStructurePage = typeof window !== 'undefined' && ['/struktur', '/struktur-organisasi'].includes(window.location.pathname.toLowerCase());
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!disposed) setFooterConfig({ copyright: dataConfig?.copyright || DEFAULT_COPYRIGHT });
      } catch (error) {
        console.warn('[Footer] setting fetch skipped:', error);
      }
    };
    void load();
    const channel = supabase.channel('public_footer_realtime_sync')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'site_settings' }, (payload: any) => {
        if (payload.new && ['footer_config', 'footer_settings'].includes(payload.new.key)) void load();
      }).subscribe();
    const handleUpdate = (event: Event) => {
      const key = (event as CustomEvent).detail?.key;
      if (!key || ['footer_config', 'footer_settings'].includes(key)) void load();
    };
    window.addEventListener('site_setting_updated', handleUpdate);
    return () => { disposed = true; void supabase.removeChannel(channel); window.removeEventListener('site_setting_updated', handleUpdate); };
  }, [isStructurePage]);

  if (isStructurePage) return null;

  return (
    <footer id="footer-section" className="w-full bg-[#050914] text-slate-400 border-t border-slate-800/80 relative z-20 shrink-0">
